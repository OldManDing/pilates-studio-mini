...
本轮针对首页、页面壳层、底部导航和共享卡片交互做了复查与修复归档。

已修复项：

- `PageShell` 已按 Figma 参考增加 `max-width: 430px` 与居中收敛，避免宽屏预览时内容横向发散。
- `custom-tab-bar` 底部留白已对齐 Figma 的 `34px + safe-area` 节奏，并同步旧 `.tab-bar-page` 预留逻辑。
- 首页空数据不再伪装成真实业务数据：
  - 无会员时展示 `--.--.--`、`--`、`--天`。
  - 无今日预约时展示待安排状态与引导文案。
  - 无近期预约时展示 `HomeUpcomingList` 空态，不再插入静态预约列表。
  - 无推荐课程时展示课程更新占位，不再伪装具体课程时间。
- 首页可点击区域已从 `View + onClick` 收口到按钮语义，并补充按压反馈：
  - 会员卡详情入口
  - 首页服务入口
  - 近期安排列表项
- `AppCard` 已为带 `onClick` 的卡片补充统一 pressed 反馈。
- TabBar item 已补充轻量按压反馈。

仍需人工 QA 关注：

- 首页 `index.scss` 仍保留一段 `Homepage fidelity refinement pass` 后置覆盖；当前视觉可用，但后续如继续大改首页，建议再做一次样式层级收口。
- 微信开发者工具与真机的 `env(safe-area-inset-bottom)` 表现可能不同，需要在至少一台带安全区设备上确认底部导航留白。
- 当前 Figma 参考为 Web 运行时，Taro 小程序里 Button / View 的按压态无法做到完全同构，需要以真机手感为最终验收依据。
...
- 这是重点页面
- 已改为基于真实可预约场次接口映射课程列表，后续重点验证接口字段与筛选体验
- 页面结构继续保留：
  - Hero
  - 日期条
  - 分类 pills
